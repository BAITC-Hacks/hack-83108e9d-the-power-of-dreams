...
The user confirmed that the four-hour implementation/demo constraint still applies. No new start time, exact deadline or remaining duration was supplied. Do not restart the clock. Preserve a 60-minute final verification/demo reserve and reconcile the actual remaining time before implementation; do not invent a latest-start timestamp or claim schedule feasibility.

## Foundation dependency and OpenSpec transfer

The P00 design supplies normalization, JSON fields, comparison context, module boundaries, failure policies and live-evidence criteria. Exact materialized types/examples and the accepted P00 commit remain implementation dependencies, not questions to redesign here.

Inspection during this discussion found untracked `foundation-and-contracts` and `contractor-selection-mvp` drafts in `D:/Alem/hack-83108e9d-the-power-of-dreams-wt-cs-00`. The foundation proposal records a subsequent user decision to make P00 a separate change and preserve the older MVP draft as reference. This supersedes the earlier proposed single-change placement for P00. These are observations of local planning files, not proof of completed or published P00. Neither draft was edited by this discussion.

Before the next OpenSpec write, reconcile the current foundation status, frozen interfaces and draft ownership. Link this approved outcome from the product proposal and carry its decisions into specs, design and P01 tasks. Preserve the reference draft and avoid a duplicate foundation change. Do not infer permission to overwrite another task's files. Later stages retain their existing acceptance obligations; deferring a UI feature here does not remove it from the MVP.
...
One coordinator owns P01 sequentially. Future branch/worktree are `codex/cs-01-slice` and `D:/Alem/hack-83108e9d-the-power-of-dreams-wt-cs-01`; verify availability and the exact accepted P00 base before creating them. The P01 application binds to `127.0.0.1:3101`. This stage-specific port does not change the general documented default of 3000. Keep build/temp output in the assigned worktree.
