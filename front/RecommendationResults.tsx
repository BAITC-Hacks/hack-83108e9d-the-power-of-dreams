...
const modes = { openai_evidence: 'ИИ выбрал цитаты из описаний; условия проверены по каталогу.', mixed: 'Часть объяснений сформирована без ИИ', catalog_fallback: 'Объяснения сформированы по полям каталога без ИИ', not_needed: '', brief_evidence: 'Порядок учитывает подтверждённые пожелания и сведения из анкет. Цитаты описывают заявления подрядчика, а не независимую проверку.' };
...
export default function RecommendationResults({ result, narrative, previous }: { result: RecommendationResponse; narrative: string[]; previous: boolean }) {
  return <div className="successful-result"><p className="successful-conditions"><strong>{previous ? 'Предыдущий результат' : 'Условия результата'}:</strong> {conditions(result.normalizedRequest)}</p>
    {result.normalizedRequest.brief && <p className="confirmed-wishes"><strong>Подтверждённые пожелания:</strong> {result.normalizedRequest.brief.conditions.map(c => `«${c.text}»`).join('; ')}.</p>}
    <p>{result.outcome === 'category_absent' ? 'В каталоге этого города нет подрядчиков выбранной категории. Измените город или категорию.' : result.outcome === 'no_match' ? 'По этим условиям подходящих вариантов нет. Проверьте условия с учётом причин исключения ниже.' : `Подходят ${result.summary.eligibleCount} из ${result.summary.candidateCount}. Показываем ${result.cards.length} ${result.explanationMode === 'brief_evidence' ? 'сначала с меньшим числом расхождений и большим числом подтверждённых совпадений с пожеланиями; затем по стартовой цене и ID.' : 'по возрастанию стартовой цены; при равной цене — по идентификатору каталога.'}`}</p>
