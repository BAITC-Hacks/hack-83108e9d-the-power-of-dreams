...
export default function ContractorForm() {
  ...
  return <main><header className="page-header"><div className="brand">join city<svg className="brand-mark" viewBox="0 0 64 32" width="56" height="28" fill="none" strokeWidth="7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m5 5 6 11-6 11" stroke="#CBE198"/><path d="m26 5 6 11-6 11" stroke="#EF8BA6"/><path d="m47 5 6 11-6 11" stroke="#86D4F2"/></svg></div><div className="page-intro"><h1>Подбор подрядчиков для мероприятий</h1><p>До трёх вариантов с объяснением каждого результата.</p></div></header>
    <div className="workspace"><details ref={conditionsPanel} className="form-panel" open aria-labelledby="conditions"><summary className="conditions-toggle"><span><h2 id="conditions">Ваше мероприятие</h2><span className="collapsed-conditions">{draft.city}{draft.date && `, ${displayDate(draft.date)}`}</span></span><span className="toggle-chevron" aria-hidden="true"/></summary>
      ...
      <section className="results" aria-labelledby="results-heading" aria-busy={!!pending}><div className="results-heading"><h2 ref={resultsHeading} tabIndex={-1} id="results-heading">Подходящие варианты</h2>{success && <span className="result-total" aria-label={`Показано карточек: ${success.result.cards.length}`}>{success.result.cards.length}</span>}</div>
        ...
        {!success && !pending && !error && <div className="placeholder"><svg className="selection-motif" viewBox="0 0 100 64" width="100" height="64" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="2" y="12" width="38" height="44" rx="8"/><circle cx="21" cy="26" r="5"/><path d="M12 43a9 9 0 0 1 18 0"/><rect x="32" y="3" width="42" height="54" rx="8" fill="var(--paper)"/><path d="M44 18h18M44 27h12M44 36h8"/><circle cx="76" cy="44" r="16" fill="var(--paper)"/><path d="m69 44 5 5 9-10"/></svg><h3>Кого ищем для вашего события?</h3><p>Укажите город, дату и условия мероприятия.<br/>Нажмите «Подобрать», чтобы увидеть варианты и объяснение каждого выбора.</p><p className="placeholder-note">Учитываем бюджет, формат и отметки занятости.</p></div>}
